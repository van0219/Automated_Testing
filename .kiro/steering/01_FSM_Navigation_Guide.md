...
### Rule 6: Skip Read-Only Fields (CRITICAL)

**NEVER attempt to populate read-only fields** - they will cause errors and waste time.

**How to identify read-only fields:**
- Look for `[disabled]` attribute in snapshot
- Look for grayed-out appearance
- Look for fields that don't have input controls (textbox, combobox)
- Common read-only fields: Voucher, Status, calculated totals
...
// Example: generic "Voucher" [ref=f1e7128]
//          paragraph [ref=f1e7130]: MV-506
// This is read-only (paragraph, not textbox)

// 3. Skip read-only fields - do NOT attempt to populate
// Move to next editable field

// 4. For editable fields, look for:
// - textbox "Field Name" [ref=...]
// - combobox "Field Name" [ref=...]
// - checkbox "Field Name" [ref=...]
```

**Common read-only fields in FSM:**
...
**Key Learning**: If a field is read-only, skip it entirely. Use lookup buttons for editable fields instead of trying to type directly.

### Rule 7: Use Search for Comprehensive Lists
...
### Rule 8: Approval Workflows Are Asynchronous (CRITICAL)

**Approval IPAs run in the BACKGROUND after submission** - custom fields update AFTER work unit completes.
...
**Validation Steps:**
1. Submit invoice/journal/transaction for approval
2. **DO NOT assume failure if custom fields show "Unsubmitted"** - this is NORMAL immediately after submission
3. Navigate to Process Server Administrator > Work Units
4. Search for approval process (by process name or transaction number)
5. Monitor work unit status until "Completed" (use adaptive polling: 10s → 30s → 60s)
6. Return to transaction form
7. Click the "Refresh" button in the form toolbar (do NOT refresh entire browser)
8. Verify status changed (e.g., "Pending Approval" → "Released")
9. Verify custom fields populated (e.g., SONH Approval Status, Work Unit Reference #)
10. Check Approval Tracking tab for approval actions

**Common Mistake**: Concluding test failed because custom fields show "Unsubmitted" immediately after submission. Always monitor work units before validating approval status.

**Key Learning**: Custom fields like "SONH Approval Status" and "Work Unit Reference #" show "Unsubmitted" or blank immediately after submission. This is NORMAL. Fields update AFTER the approval work unit completes (5-10 minutes). Always monitor work units to verify completion before validating approval results.
